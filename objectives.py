...
def _format_objective_short(obj: dict) -> str:
	obj_type = (obj.get("type") or "").strip()
	remove_at_ts = int(obj.get("remove_at_ts") or 0)
	if remove_at_ts:
		return f"{_format_objective_name(obj)} — has already popped up, it will be removed soon."

	if obj_type == _OBJECTIVE_TYPE_VORTEX:
		rarity = _vortex_rarity_display(obj.get("rarity"))
		map_name = obj.get("map") or "Unknown map"
		ts = int(obj.get("pop_at_ts") or 0)
		pop_part = _format_pop_time(ts, obj.get("pop_time_utc"))
		return f"🌀 Vortex ({rarity}) — {map_name} — {pop_part}"
	if obj_type == _OBJECTIVE_TYPE_NODE:
		node_type = obj.get("node_type") or "Unknown"
		tier = obj.get("tier") or "?"
		map_name = obj.get("map") or "Unknown map"
		ts = int(obj.get("pop_at_ts") or 0)
		pop_part = _format_pop_time(ts, obj.get("pop_time_utc"))
		return f"⛏️ Node ({node_type} {tier}) — {map_name} — {pop_part}"
...
def _format_objective_name(obj: dict) -> str:
	...
	if obj_type == _OBJECTIVE_TYPE_NODE:
		...
		tier = obj.get("tier") or "?"
		map_name = obj.get("map") or "Unknown map"
		return f"⛏️ Node ({node_type} {tier}) — {map_name}"
	return "Objective"
...
async def _process_all_guilds(bot: discord.Client, now_ts: int) -> None:
	...
	for guild_id_str, entry in list(config.items()):
		...
		popped_remove_at_by_key, remove_keys, needs_panel_refresh = await _process_guild(guild, entry, now_ts)
		if popped_remove_at_by_key or remove_keys:
			_apply_objective_deltas(guild_id, popped_remove_at_by_key, remove_keys)
...
def _apply_objective_deltas(guild_id: int, popped_remove_at_by_key: dict[str, int], remove_keys: set[str]) -> None:
...
async def _process_guild(
	...
	return popped_remove_at_by_key, remove_keys, needs_panel_refresh
...
def _validate_step(view: ObjectiveWizardView) -> Optional[str]:
	if view.step == 1:
		if view.state.objective_type not in (_OBJECTIVE_TYPE_VORTEX, _OBJECTIVE_TYPE_NODE):
...
class _WizardConfirmButton(discord.ui.Button):
	...
	async def callback(self, interaction: discord.Interaction) -> None:
		...
		await interaction.response.edit_message(content="Objective posted.", embed=None, view=None)
